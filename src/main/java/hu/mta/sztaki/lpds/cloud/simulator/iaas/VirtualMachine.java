...
public class VirtualMachine extends MaxMinConsumer {
    ...
    private StorageObject identifyWWS(final String id, final double dirtyingRate) {
        return new StorageObject(id, (long) ((getTotalMemoryPages() * pageSize) * dirtyingRate), false);
    }
     ...
     * @param vasource the repository where the VA for this VM is found. If
     * null, the function assumes it is found in the hosting PM's repository.
     * @throws StateChangeException if the VM is not destroyed
     * @throws VMManagementException if the VA transfer failed and the state
     * change was reverted
     */
     ...
     *
     * @param pm the physical machine that hosts the VM.
     * @param vasource the repository where the VA for this VM is found. If
     * null, the function assumes it is found in the hosting PM's repository.
     * @param es The way the VM's state should be changed the function will fire
     * an event on this channel if the VA is cloned properly
     * @throws VMManagementException if the VA transfer failed and the state
     * change was reverted
     */
     ...
     * should take place
     * @throws VMManagementException in case some errors were reported during
     * the resume operation at the new physical machine
     */
        ...
        class EndRoundEvent extends ConsumptionEventAdapter {
            ...
            public void conComplete() {
                ...
                if (savedmemory.size > WWS_MAX_SIZE && rounds < numRound) {
                    ...
                    try {
                        suspendLive(new EventSetup(State.MIGRATING) {
                            @Override
                            public void changeEvents() {
                                super.changeEvents();
                                try {
     ...
     * or memory states.
     *
     * @throws StateChangeException if some parts of the VM are under transfer
     * so it cannot be determined what to clean up.
     */
     ...
     * memory state (first locally, then locally/remotely depending on the VM
     * storage scenario
     */
     ...
     *
     * @param ev the eventsetup hook to be used when the suspend operation is
     * complete.
     * @throws StateChangeException see at regular suspend
     * @throws VMManagementException see at regular suspend
     */
     ...
     * @throws VMManagementException see at regular suspend
     */
    private void suspendLive(final EventSetup ev) throws VMManagementException,
            NetworkNode.NetworkException {
        if (currState != State.RUNNING) {
            throw new StateChangeException(
                    "Cannot suspend a not running machine");
        ...
        List<ResourceConsumption>[] completeConlist = new List[]{
            underProcessing, new ArrayList<ResourceConsumption>(toBeAdded)};
        double dirtyingRate = getTotalDirtyingRate();
        ...
        savedmemory = new StorageObject(tmemid, ra.allocated.requiredMemory,
                false);
        savedmemory = identifyWWS(tmemid, dirtyingRate);
        if (!pmdisk.registerObject(savedmemory)) {
            throw new VMManagementException(
                    "Not enough space on localDisk for the suspend operation of "
                    + savedmemory);
     ...
     * @throws VMManagementException if there is not enough space to retreive
     * the memory state to the PM's repository
     */
